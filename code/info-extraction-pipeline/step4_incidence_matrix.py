...
# Placeholder defaults. run_pipeline.step_build_incidence() overwrites every
# directory constant and every tuning threshold in this module from
# pipeline_config.yaml before calling build_image().
...
# Combines the per-image artifacts
#   component_bbox/<image>.txt   bbox + class for each component
#   orientation/<image>.json     orientation 'u'/'r'/'d'/'l' for required classes
#   node_touches/<image>.json    per-node touches (component_bbox_idx, edge,
#                                contact_xy) + removed_single_bbox_nodes
#                                (potential ports)
# into one net name per component pin, plus benign `flags` and blocking
# `red_flags` for anything that could not be resolved. run_pipeline writes the
# result to incidence_matrix/<image>.json; the netlist step refuses to emit a
# .cir for any image that still carries a red_flag.
