...
def make_step1_args(config, stems: list[str] | None, device: torch.device, force_ocr: bool):
...
def load_step1(config):
    module = import_module_from_path(
        "step1_component_line_detection",
        config.paths.repo_root / "step1_component_line_detection.py",
    )
    ...
    # Optional GUI hook: if <output_dir>/manual_jj_overrides/{stem}.json exists,
...
def step_detect_components(config, stems: list[str] | None, device: torch.device, force_ocr: bool) -> None:
    step1 = load_step1(config)
    args = make_step1_args(config, stems, device, force_ocr)
    step1.run_yolo_and_mask(args, device)
...
def step_detect_orientation(config, stems: list[str] | None, device: torch.device) -> None:
    orient_mod = import_module_from_path(
        "step2_component_orientation", config.paths.repo_root / "step2_component_orientation.py"
    )
...
def step_extract_lines(config, stems: list[str] | None, device: torch.device, force_ocr: bool) -> None:
    ...
    step1 = load_step1(config)
    args = make_step1_args(config, stems, device, force_ocr)
    step1.run_combined_stage(device, args)
...
def step_generate_nodes(config, stems: list[str] | None, device: torch.device, force_ocr: bool) -> None:
    ...
    step1 = load_step1(config)
    args = make_step1_args(config, stems, device, force_ocr)
    step1.run_component_anchor_nodes(args)


def step_export_touches(config, stems: list[str] | None) -> None:
    touch_mod = import_module_from_path(
        "step3_node_component_touches",
        config.paths.repo_root / "step3_node_component_touches.py",
    )
...
def step_build_incidence(config, stems: list[str] | None) -> None:
    inc_mod = import_module_from_path(
        "step4_incidence_matrix", config.paths.repo_root / "step4_incidence_matrix.py"
    )
...
def step_build_netlist(config, stems: list[str] | None) -> None:
    net_mod = import_module_from_path(
        "step5_netlist", config.paths.repo_root / "step5_netlist.py"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("all", *STEPS),
        default="all",
        help="single step to run, or all steps; default runs through build_netlist",
    )
    parser.add_argument(
        "--from-step",
        choices=STEPS,
        help="run from this step through build_netlist",
    )
    ...
    parser.add_argument(
        "--no-copy-inputs",
        action="store_true",
        help="require inputs to already live in <output_dir>/images",
    )
...
def main() -> None:
    ...
    if args.from_step and args.step != "all":
        raise ValueError("Use either --step for one step or --from-step for a step range, not both.")
