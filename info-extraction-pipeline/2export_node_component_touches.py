#!/usr/bin/env python3
"""Export component-touch endpoints for experiment-1 nodes.

This script reads the data produced by pipeline.py::run_experiment1_component_anchor_nodes
and writes a compact JSON file per image. Each touch keeps the anchor line's
actual endpoint that is closest to the component-edge contact point.
"""
...
DEFAULT_EXP1_DATA_DIR = (
    ROOT / "passedTest" / "experiment1_component_anchor_nodes" / "data"
)
DEFAULT_BBOX_DIR = ROOT / "maskedImages" / "_bboxes"
DEFAULT_OUTPUT_DIR = (
    ROOT / "passedTest" / "experiment1_component_anchor_nodes" / "node_touches"
)
...
def parse_args():
    ...
    p.add_argument(
        "--data-dir",
        type=Path,
        default=DEFAULT_EXP1_DATA_DIR,
        help="directory containing experiment1 node .npz/.json files",
    )
    p.add_argument(
        "--bbox-dir",
        type=Path,
        default=DEFAULT_BBOX_DIR,
        help="directory containing component bbox txt files",
    )
    p.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="directory where node touch json files will be written",
    )
    p.add_argument(
        "--stem",
        action="append",
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "deprecated; touches are always aggregated by "
            "node_id + component_bbox_idx + edge"
        ),
    )
    p.add_argument(
        "--extra-endpoint-extend-px",
        type=float,
        default=10.0,
        help=(
            "extend node endpoints outward by this many pixels to find "
            "additional component touches"
        ),
    )
    p.add_argument(
        "--keep-single-bbox-nodes",
        action="store_true",
        help="do not remove nodes that touch only one distinct component bbox",
    )
    return p.parse_args()
...
def iter_stems(data_dir, selected_stems):
    if selected_stems:
...
def main():
    ...
    stems = list(iter_stems(data_dir, args.stem))
    if not stems:
        print(f"[node-touch] no .npz files found in {data_dir}")
        return
    ...
    for stem in stems:
        ...
        if not npz_path.exists():
            print(f"[node-touch] {stem}: missing {npz_path}, skipping")
            continue
        if not json_path.exists():
            print(f"[node-touch] {stem}: missing {json_path}, skipping")
            continue

        out_path, n_touches, n_nodes, n_added, warnings = export_one(
            stem=stem,
            npz_path=npz_path,
            json_path=json_path,
            ...
            dedupe=args.dedupe,
            extra_endpoint_extend_px=args.extra_endpoint_extend_px,
            keep_single_bbox_nodes=args.keep_single_bbox_nodes,
        )
        written += 1
        suffix = f", warnings={len(warnings)}" if warnings else ""
        print(
            f"[node-touch] {stem}: touches={n_touches} nodes={n_nodes} "
            f"added_extended={n_added} -> {out_path}{suffix}"
        )

    print(f"[node-touch] wrote {written} file(s) to {output_dir}")
