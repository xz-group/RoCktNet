#!/usr/bin/env python3
"""End-to-end schematic-parsing pipeline:

1. YOLO component detection (componentDetection/best.pt) on every image in
   testData/images. Each detected component bbox is masked out (filled
   white/black) and saved to maskedImages/; bbox metadata is persisted to
   maskedImages/_bboxes/<stem>.txt.

2.  The main line-extraction stage on the text-intact masked
   image. Always populates the OCR text-bbox cache first (idempotent), then
   per image:
      a. Local-mean adaptive binarization of the masked grayscale image.
      b. HAWPv3 -> keep lines with score >= --combined-hawp-threshold.
      c. Clip each line against the YOLO component bboxes (subtract the
         inside-bbox portion); drop sub-segments shorter than
         --min-line-length.
      d. Whitespace filter: drop any line whose sampled pixels are mostly
         white background (>= --whitespace-fraction of pixels above
         --whitespace-threshold).
      e. Black-pixel augmentation: skeletonize remaining black pixels that
         lie outside component bboxes and outside OCR text bboxes (with
         --text-distance clearance), then Hough-extract extra lines from
         that residual mask and merge them with the HAWP-derived set.
   Outputs -> combinedResultsF1/lines/ (npz + txt) and
   combinedResultsF1/visualizations/.

3.  node generation. For each final line set:
      a. Find lines whose endpoint is nearest to a component bbox edge,
         extend the endpoint toward that edge, and validate the anchor by
         checking for connecting black pixels just inside the bbox.
      b. Union-find: group anchored lines (plus other lines within
         ---node-union-dist) into nodes.
...
HAWP_RESULTS_DIR = ROOT / "hawpResults"
HAWP_LINES_DIR = HAWP_RESULTS_DIR / "lines"
HAWP_VIS_DIR = HAWP_RESULTS_DIR / "visualizations"
...
# Optional path supplied by run_pipeline (or any caller). If set and a
# {stem}.json file is present there, its added_jumps / added_junctions are
# unioned with the YOLO output in run_component_anchor_nodes. Default None
...
def parse_args():
    ...
    p.add_argument(
        "--iou", type=float, default=0.45, help="YOLO NMS IoU threshold (default 0.45)"
    )
    p.add_argument(
        "--pad",
        type=int,
        default=1,
        help="extra pixels to pad the bbox before masking (default 1)",
    )
    p.add_argument(
        "--mask-color",
        default="white",
        choices=["white", "black"],
        help="fill color used to erase detected components (default white)",
    )
    p.add_argument(
        "--skip-yolo",
        action="store_true",
        help="reuse existing maskedImages/ instead of re-running YOLO",
    )
    p.add_argument(
        "--skip-hawp",
        action="store_true",
        help="skip the plain HAWPv3 wireframe-parsing stage",
    )
    p.add_argument(
        "--hawp-threshold",
        type=float,
        default=0.05,
        help="line-score cutoff for HAWP visualization (default 0.05, "
        "matches HAWP's official predict.py)",
    )
    p.add_argument(
        "--remove-text",
        action="store_true",
        help="run EasyOCR over the YOLO-masked images and white-out "
        "detected text bboxes; downstream line-detection stages "
        "then read from maskedNoTextImages/ instead of maskedImages/",
    )
    p.add_argument(
        "--ocr-pad",
        type=int,
        default=2,
        help="extra pixels around each OCR text bbox before whiting "
        "it out (default 2)",
    )
    p.add_argument(
        "--ocr-conf",
        type=float,
        default=0.2,
        help="minimum OCR confidence to count a text region (default 0.2)",
    )
    p.add_argument(
        "--force-ocr",
        action="store_true",
        help="re-run OCR even if maskedNoTextImages/ already has a cached "
        "result; without this flag, images that already have a text-removed "
        "counterpart are skipped",
    )
    # ----- combined HAWP + bbox clip + text filter stage -----
    p.add_argument(
        "--combined",
        action="store_true",
        help="run the combined stage: HAWP on text-intact masked image, then "
        "clip lines against component bboxes (keep only the portions outside "
        "bboxes), then drop lines that overlap or are close to OCR-detected "
        "text bboxes.",
    )
    p.add_argument(
        "--combined-hawp-threshold",
        type=float,
        default=0.05,
        help="HAWP score threshold for the combined stage (default 0.05)",
    )
    p.add_argument(
        "--text-distance",
        type=float,
        default=5.0,
        help="Drop a line if its minimum distance to any OCR text bbox is "
        "<= this many pixels (0 = overlap, > 0 = near). Default 5 px.",
    )
    p.add_argument(
        "--min-line-length",
        type=float,
        default=4.0,
        help="After clipping against bboxes, drop sub-segments shorter than "
        "this many pixels (default 4)",
    )
    p.add_argument(
        "--whitespace-threshold",
        type=int,
        default=240,
        help="grayscale value (0-255) at or above which a pixel counts as "
        "'white background' for the whitespace filter (default 240)",
    )
    p.add_argument(
        "--whitespace-fraction",
        type=float,
        default=0.8,
        help="Drop a line if >= this fraction of its sampled pixels are "
        "whitespace, i.e. the line lies on blank background (default 0.8; "
        "set to 1.0 to disable the whitespace filter).",
    )
    p.add_argument(
        "--generate-node",
        action="store_true",
        help="run the experiment 1 component-anchor node generation stage "
        "after the combined stage (only if --combined is also set)",
    )
    p.add_argument(
        "---perp-tol",
        type=float,
        default=10.0,
        help="degrees of tolerance around 90 deg for the perpendicular "
        "(jump-style) crossing test (default 10).",
    )
    p.add_argument(
        "---jj-conf",
        type=float,
        default=0.25,
        help="confidence threshold for the junction/jump YOLO model " "(default 0.25).",
    )
    p.add_argument(
        "---jj-bbox-pad",
        type=float,
        default=2.0,
        help="extra pixels to expand each junction/jump bbox when testing "
        "whether a line-line contact point falls inside (default 2).",
    )
    p.add_argument(
        "---crossover-probe-dist",
        type=float,
        default=5.0,
        help="in the implicit-jump area, distance in pixels to probe "
        "outward from the contact along each line's direction when "
        "deciding if a 90-deg touch is a real crossover (default 5).",
    )
    p.add_argument(
        "---crossover-probe-tol",
        type=float,
        default=3,
        help="how close (in pixels) a line in the raw node must come to "
        "a probe point to count as 'present' there (default 2).",
    )
    p.add_argument(
        "---crossover-min-line-len",
        type=float,
        default=3.0,
        help="minimum length in pixels a candidate line must have to "
        "count when probing for a crossover (default 3).",
    )
    p.add_argument(
        "---jump-parallel-tol",
        type=float,
        default=20.0,
        help="inside an explicit jump bbox, any pair of close segments "
        "whose angle exceeds this many degrees (i.e. they are NOT "
        "roughly parallel) is split. Smaller values keep more pairs "
        "merged inside jump areas; larger values are more aggressive "
        "about separating different-direction wires (default 20).",
    )
    p.add_argument(
        "--stem",
        action="append",
        default=None,
        help="only process this image stem; may be passed multiple times",
    )
    return p.parse_args()


# --------------------------------------------------------------------------- #
# Stage 1: YOLO detection + component masking                                  #
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Stage 1b: OCR text removal (optional, --remove-text)                         #
# --------------------------------------------------------------------------- #
def run_ocr_text_removal(device, pad=2, conf_thresh=0.2, force=False, stems=None):
    ...
    text bbox. Saves the cleaned images to MASKED_NOTEXT_DIR and persists the
    bbox metadata under MASKED_NOTEXT_DIR/_text_bboxes/ so the cleanup is
    reproducible / inspectable.

    Caching: incremental by default - any image that already has both a cleaned
    image and bbox metadata on disk is skipped. Pass force=True (CLI:
    --force-ocr) to redo everything (e.g. after changing pad/conf)."""
    import cv2
    ...
    try:
        ...
        raise RuntimeError(
            "easyocr is required for --remove-text. "
...
# --------------------------------------------------------------------------- #
# Shared helper: load YOLO bbox metadata produced by run_yolo_and_mask         #
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Stage 3: HAWPv3 wireframe parsing                                            #
# --------------------------------------------------------------------------- #
...
def save_hawp_visualization(im, lines, scores, threshold, out_path, title=""):
    """Single-pane visualization mirroring the Netlistify style: red lines for
    accepted detections (score >= threshold) + cyan endpoint dots."""
...
def run_hawp_stage(device, threshold=0.05, masked_input=None):
    if masked_input is None:
        masked_input = MASKED_DIR
    if not HAWP_WEIGHTS.exists():
        print(f"[HAWP] checkpoint not found at {HAWP_WEIGHTS} - skipping.")
        return
    import cv2
    ...
    masked = sorted(
        [
            p
            for p in masked_input.iterdir()
            if p.is_file() and p.suffix.lower() in IMG_EXTS
        ]
    )
    print(f"[HAWP] found {len(masked)} masked images in {masked_input}")

    HAWP_LINES_DIR.mkdir(parents=True, exist_ok=True)
    HAWP_VIS_DIR.mkdir(parents=True, exist_ok=True)

    for img_path in masked:
        print(f"[HAWP] processing {img_path.name}")
        im_gray = cv2.imread(str(img_path), 0)
        if im_gray is None:
            print(f"[HAWP] skipping unreadable {img_path}")
            continue

        lines, scores = run_hawp_on_image(model, im_gray, device)

        np.savez(HAWP_LINES_DIR / f"{img_path.stem}.npz", lines=lines, scores=scores)
        flat = np.zeros((len(lines), 5), dtype=np.float32)
        if len(lines):
            flat[:, 0] = lines[:, 0, 0]
            flat[:, 1] = lines[:, 0, 1]
            flat[:, 2] = lines[:, 1, 0]
            ...
            flat[:, 4] = scores
        np.savetxt(
            HAWP_LINES_DIR / f"{img_path.stem}.txt",
            flat,
            fmt="%.2f %.2f %.2f %.2f %.4f",
            header="x1 y1 x2 y2 score",
            comments="",
        )

        # Use the masked RGB image as the canvas so this looks comparable to
        # the LCNN / Netlistify visualizations.
        im_rgb = cv2.cvtColor(im_gray, cv2.COLOR_GRAY2RGB)
        save_hawp_visualization(
            im_rgb,
            lines,
            scores,
            threshold,
            HAWP_VIS_DIR / f"{img_path.stem}.png",
            title=f"{img_path.name}  (>= {threshold})",
        )
...
def _draw_lines_to_mask(shape, lines, thickness=3):
    """
    Rasterize line segments into a binary mask.
    """
    import cv2
    import numpy as np
    ...
    for ln in lines:
        ...
        p1 = (
            int(np.clip(round(float(x1)), 0, W - 1)),
            int(np.clip(round(float(y1)), 0, H - 1)),
        )
        p2 = (
            int(np.clip(round(float(x2)), 0, W - 1)),
            int(np.clip(round(float(y2)), 0, H - 1)),
        )
...
def _group_lines_into_nodes_by_anchor_union_find(
    ...
    has_x_jump = len(x_jump_bboxes) > 0
...
def run_combined_stage(device, args):
    ...
    Inputs per image:
        maskedImages/<name>.jpg            (text-intact)        -> HAWPv3 input
        maskedImages/_bboxes/<name>.txt    (YOLO bboxes)        -> clipping
        maskedNoTextImages/_text_bboxes/<name>.txt              -> text filter
                                           (populated on demand)
...
def run_component_anchor_nodes(args):
    ...
    for line_path in line_files:
        ...
        # Junction/jump detection on the masked image (always the
        # text-intact maskedImages/<stem>, regardless of --remove-text).
        jj_img_path = None
        for ext in IMG_EXTS:
...
def main():
    ...
    if not args.skip_yolo:
        run_yolo_and_mask(args, device)
    else:
        print("[YOLO] skipped (using existing maskedImages/)")
    ...
    if args.remove_text:
        ...
        masked_input = MASKED_NOTEXT_DIR
        print("[done] text-removed images:", MASKED_NOTEXT_DIR)
    else:
        print("[OCR] skipped (use --remove-text to strip text first)")

    run_combined_stage(device, args)
    print("[done] combined visualizations:", COMBINED_VIS_DIR)
    print("[done] combined line segments:", COMBINED_LINES_DIR)
    run_component_anchor_nodes(args)
