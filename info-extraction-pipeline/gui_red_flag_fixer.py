#!/usr/bin/env python3
"""GUI for semi-automatic red-flag fixing via pipeline parameter tweaking.

Workflow:
  1. The left panel lists images that came out of the pipeline with red_flags
     (toggle off "only red-flagged" to see everything).
  2. Pick one or more stems.
  3. Adjust parameters in the right-hand panel (grouped by pipeline step).
  4. Click "Re-run from <step>" — the GUI spawns run_pipeline.py with the
     modified session config, scoped to the selected stems and --from-step.
  5. Visualizations and the red-flag banner refresh when the run finishes.

The session config lives next to pipeline_config.yaml as
.gui_session_config.yaml (it must sit at the repo root because
pipeline_common.py resolves relative paths against the config file's
parent). The source pipeline_config.yaml is never modified unless you press
"Save as defaults".
"""
...
RESULT_DIR = REPO_ROOT / "resultImage2Net"
...
# ---------- touches IO + edit helpers (Phase 2d) ----------
...
# ---------- manual jump/junction overrides (Phase 2c, Mode 1) ----------
#
...
# them to result/manual_jj_overrides/{stem}.json; 1pipeline._apply_manual_jj_
# overrides reads that file and unions the entries with the YOLO output before
# building line_node_ids. The override file is purely additive.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Schematic Pipeline – red-flag fixer")
