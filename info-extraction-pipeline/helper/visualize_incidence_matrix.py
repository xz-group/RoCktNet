"""
visualize_incidence_matrix.py

Draws a visual overlay for incidence_matrix/<image>.json on top of images/<image>.*.

The overlay shows:
  - component bboxes and component names
  - final pin -> net assignments from the incidence matrix
  - colored touch points grouped by final net
  - straight guide lines between touch points that belong to the same final net

Usage:
  python visualize_incidence_matrix.py --image Book294
  python visualize_incidence_matrix.py --all
"""
...
OUT_DIR = ROOT / "incidence_visualization"
...
def main():
    ...
    parser.add_argument("--image", help="single image id, e.g. Book294")
    parser.add_argument("--all", action="store_true", help="visualize every incidence_matrix/*.json")
    parser.add_argument("--out-dir", default=str(OUT_DIR), help="output directory")
    args = parser.parse_args()

    if not args.image and not args.all:
        parser.error("provide --image <id> or --all")

    out_dir = Path(args.out_dir)
    image_ids = [args.image] if args.image else sorted(p.stem for p in INCIDENCE_DIR.glob("*.json"))

    for image_id in image_ids:
        out_path = visualize_image(image_id, out_dir)
        print(f"[OK] {image_id} -> {out_path}")
