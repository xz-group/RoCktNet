#!/usr/bin/env python3
"""Visualize node-to-component touches exported by export_node_component_touches.py."""
...
DEFAULT_OUTPUT_DIR = (
    ROOT
    / "passedTest"
    / "experiment1_component_anchor_nodes"
    / "node_touch_visualizations"
)
DEFAULT_IMAGE_DIR = ROOT / "maskedImages"
...
def parse_args():
    ...
    p.add_argument(
        ...
        help="directory containing exported node touch json files",
    )
    p.add_argument(
        "--image-dir",
        type=Path,
    ...
    p.add_argument(
        "--no-labels",
        action="store_true",
        help="do not draw node labels beside touch points",
    )
    p.add_argument(
        "--show-endpoints",
        action="store_true",
        help="also draw the source endpoint and a thin endpoint-to-contact line",
    )
    return p.parse_args()
...
def node_color(node_id):
    ...
    # Deterministic, well-spaced colors in HSV.
...
def iter_touch_files(touch_dir, selected_stems):
    if selected_stems:
        for stem in selected_stems:
            yield touch_dir / f"{stem}.json"
        return

    yield from sorted(touch_dir.glob("*.json"))


def main():
    args = parse_args()

    touch_files = list(iter_touch_files(args.touch_dir, args.stem))
    if not touch_files:
        print(f"[touch-vis] no json files found in {args.touch_dir}")
        return

    written = 0
    for touch_path in touch_files:
        if not touch_path.exists():
            print(f"[touch-vis] missing {touch_path}, skipping")
            continue
        try:
            out_path, n_touches = visualize_one(
            ...
            print(f"[touch-vis] {touch_path.stem}: {exc}, skipping")
            continue
        written += 1
        print(f"[touch-vis] {touch_path.stem}: touches={n_touches} -> {out_path}")

    print(f"[touch-vis] wrote {written} file(s) to {args.output_dir}")
